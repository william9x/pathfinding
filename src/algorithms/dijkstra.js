...
//Animations 
export async function animateDijkstra(visitedNodesInOrder, nodesInShortestPathOrder, findingPathSpeed) {
  for (let i = 0; i <= visitedNodesInOrder.length; i++) {
    if (i === visitedNodesInOrder.length) {
      await animateShortestPath(nodesInShortestPathOrder);
      await sleep(findingPathSpeed);
      return;
    }
    const node = visitedNodesInOrder[i];

    //Super lag because have to render everything again
    //this.grid[node.row][node.col].isVisited = true;
    //Instead add classname
    let el = document.getElementById(`node-${node.row}-${node.col}`);
    if (
      !el.classList.contains("node-start") &&
      !el.classList.contains("node-finish")
    ) {
